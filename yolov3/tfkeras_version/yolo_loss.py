...
def _create_mesh_xy(batch_size, grid_h, grid_w, n_box):  #生成带序号的网格
    mesh_x = tf.cast(
        tf.reshape(tf.tile(tf.range(grid_w), [grid_h]),
                   (1, grid_h, grid_w, 1, 1)), tf.float32)
    mesh_y = tf.transpose(mesh_x, (0, 2, 1, 3, 4))
    mesh_xy = tf.tile(tf.concat([mesh_x, mesh_y], -1),
                      [batch_size, 1, 1, n_box, 1])
    return mesh_xy


#将网格信息融入坐标，置信度做sigmoid
def adjust_pred_tensor(y_pred):
    grid_offset = _create_mesh_xy(*y_pred.shape[:4])
    pred_xy = grid_offset + tf.sigmoid(
        y_pred[..., :2])  #计算该尺度矩阵上的坐标sigma(t_xy) + c_xy
    ...
    preds = tf.concat(
        [pred_xy, pred_wh,
         tf.expand_dims(pred_conf, axis=-1), pred_classes],
        axis=-1)
    return preds
...
def _create_mesh_anchor(anchors, batch_size, grid_h, grid_w, n_box):
    ...
    mesh_anchor = tf.reshape(
        mesh_anchor, [batch_size, grid_h, grid_w, n_box, 2])  #每个候选框有2个值
    mesh_anchor = tf.cast(mesh_anchor, tf.float32)
    return mesh_anchor


def conf_delta_tensor(y_true, y_pred, anchors, ignore_thresh):
    pred_box_xy, pred_box_wh, pred_box_conf = y_pred[..., :2], y_pred[
        ..., 2:4], y_pred[..., 4]
    #带有候选框的格子矩阵
    anchor_grid = _create_mesh_anchor(
        anchors, *y_pred.shape[:4])  #y_pred.shape为（2，13，13，3，15）
    true_wh = y_true[:, :, :, :, 2:4]
    true_wh = anchor_grid * tf.exp(true_wh)
    true_wh = true_wh * tf.expand_dims(y_true[:, :, :, :, 4], 4)  #还原真实尺寸，高和宽
    anchors_ = tf.constant(anchors,
                           dtype='float',
                           shape=[1, 1, 1, y_pred.shape[3],
                                  2])  #y_pred.shape[3]为候选框个数
...
def loss_coord_tensor(object_mask, pred_box, true_box, wh_scale, xywh_scale):
    ...
    loss_xy = tf.reduce_sum(tf.square(xy_delta),
                            list(range(1, 5)))  #按照1，2，3，4（xyhw）规约求和
    return loss_xy

#是否存在目标，预测损失
def loss_conf_tensor(object_mask, pred_box_conf, true_box_conf, obj_scale,
                     noobj_scale, conf_delta):
    ...
    loss_conf = tf.reduce_sum(tf.square(conf_delta),
                              list(range(1, 4)))  #按照1，2，3（候选框）归约求和，0为批次
    return loss_conf

#类别预测损失
def loss_class_tensor(object_mask, pred_box_class, true_box_class,
                      class_scale):
...
def lossCalculator(y_true, y_pred, anchors, image_size):  #image_size【h,w】
    ...
    loss_box = loss_coord_tensor(object_mask, preds[..., :4], y_true[..., :4],
                                 wh_scale, xywh_scale)
    loss_conf = loss_conf_tensor(object_mask, preds[..., 4], y_true[..., 4],
                                 obj_scale, noobj_scale, conf_delta)
    loss_class = loss_class_tensor(object_mask, preds[..., 5:],
                                   y_true[..., 5:], class_scale)
...
def loss_fn(list_y_trues, list_y_preds, anchors, image_size):
    ...
    losses = [
        lossCalculator(list_y_trues[i], list_y_preds[i], inputanchors[i],
                       image_size) for i in range(len(list_y_trues))
    ]
    return tf.sqrt(tf.reduce_sum(losses))  # 将三个矩阵（三个尺度）的loss相加再开平方
